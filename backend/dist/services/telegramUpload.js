 ...
 */
async function safeEditMessage(client, chatId, params) {
...
// 状态操作序列化器
const statusActionLocks = new Map();
const lastSilentNotificationTimeMap = new Map();
...
const lastStatusMessageIdMap = new Map();
/**
 * 安全删除并追踪最后一条状态消息
 */
async function deleteLastStatusMessage(client, chatId) {
    if (!chatId)
        return;
    const chatIdStr = chatId.toString();
 ...
 * 更新最后一条状态消息 ID
 */
function updateLastStatusMessageId(chatId, msgId) {
    if (!chatId || !msgId)
        return;
...
const mediaGroupQueues = new Map();
...
// 下载并保存文件
async function downloadAndSaveFile(client, message, fileName, targetDir, onProgress) {
...
// 处理单个文件上传（带重试机制）
async function processFileUpload(client, file, queue) {
    file.status = 'queued';
...
// 处理批量文件上传队列
async function processBatchUpload(client, mediaGroupId) {
    const queue = mediaGroupQueues.get(mediaGroupId);
    if (!queue || queue.processingStarted)
        return;
...
export async function handleCleanupCallback(cleanupId) {
...
export async function handleFileUpload(client, event) {
