...
// Aliyun OSS 存储实现
export class AliyunOSSStorageProvider {
    id;
    name = 'aliyun_oss';
    client;
...
export class OneDriveStorageProvider {
    ...
    accessToken = null;
    tokenExpiresAt = 0;
    ONEDRIVE_FOLDER = 'FoomClous'; // 存储文件夹名
    ...
    async ensureFolderExists(token) {
        try {
            await axios.get(`https://graph.microsoft.com/v1.0/me/drive/root:/${this.ONEDRIVE_FOLDER}`, { headers: { 'Authorization': `Bearer ${token}` } });
        }
        catch (error) {
    ...
    async saveFile(tempPath, fileName, mimeType) {
        ...
        try {
            ...
            if (fileSize < 4 * 1024 * 1024) {
                ...
                const response = await axios.put(`https://graph.microsoft.com/v1.0/me/drive/root:/${this.ONEDRIVE_FOLDER}/${encodedFileName}:/content`, fileBuffer, {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                ...
                // 1. 创建上传会话
                const sessionRes = await axios.post(`https://graph.microsoft.com/v1.0/me/drive/root:/${this.ONEDRIVE_FOLDER}/${encodedFileName}:/createUploadSession`, {
                    item: {
                ...
                // 如果最后响应没有ID，查询文件信息
                const itemRes = await axios.get(`https://graph.microsoft.com/v1.0/me/drive/root:/${this.ONEDRIVE_FOLDER}/${encodedFileName}`, {
    ...
    async getFileStream(storedPath) {
        const token = await this.getAccessToken();
        try {
            const response = await axios.get(`https://graph.microsoft.com/v1.0/me/drive/items/${storedPath}/content`, {
                headers: { 'Authorization': `Bearer ${token}` },
                responseType: 'stream',
                timeout: 60000
            });
    ...
    async getPreviewUrl(storedPath) {
        ...
        try {
            ...
            const response = await axios.get(`https://graph.microsoft.com/v1.0/me/drive/items/${storedPath}`, {
                headers: { 'Authorization': `Bearer ${token}` },
                timeout: 30000
            });
    ...
    async deleteFile(storedPath) {
        const token = await this.getAccessToken();
        try {
            await axios.delete(`https://graph.microsoft.com/v1.0/me/drive/items/${storedPath}`, {
                headers: { 'Authorization': `Bearer ${token}` },
                timeout: 30000
            });
    ...
    async getFileSize(storedPath) {
        const token = await this.getAccessToken();
        try {
            const response = await axios.get(`https://graph.microsoft.com/v1.0/me/drive/items/${storedPath}?$select=size`, {
                headers: { 'Authorization': `Bearer ${token}` },
                timeout: 30000
            });
    ...
    async createShareLink(storedPath, password, expiration) {
        ...
        try {
            ...
            const response = await axios.post(`https://graph.microsoft.com/v1.0/me/drive/items/${storedPath}/createLink`, body, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                },
...
export class StorageManager {
        ...
        try {
            ...
            for (const row of accountsRes.rows) {
                ...
                if (row.type === 'onedrive') {
                    provider = new OneDriveStorageProvider(row.id, config.clientId, config.clientSecret || globalSecret || '', config.refreshToken, config.tenantId || 'common');
                    this.providers.set(`onedrive:${row.id}`, provider);
                }
                else if (row.type === 'aliyun_oss') {
                    provider = new AliyunOSSStorageProvider(row.id, config.region, config.accessKeyId, config.accessKeySecret, config.bucket);
    ...
    async migrateLegacyConfig() {
        ...
        if (clientId && refreshToken) {
            ...
            if (existing.rows.length === 0) {
                const insertRes = await query(`INSERT INTO storage_accounts (type, name, config, is_active) 
                     VALUES ($1, $2, $3, $4) RETURNING id`, ['onedrive', 'Default Account', JSON.stringify({ clientId, clientSecret, refreshToken, tenantId }), true]);
            ...
            const updateRes = await query('UPDATE files SET storage_account_id = $1 WHERE source = $2 AND storage_account_id IS NULL', [accountId, 'onedrive']);
    ...
    async addOneDriveAccount(name, clientId, clientSecret, refreshToken, tenantId = 'common') {
        ...
        if (existing.rows.length > 0) {
            ...
            await query('UPDATE storage_accounts SET name = $1, config = $2, updated_at = NOW() WHERE id = $3', [name, config, targetId]);
        }
        else {
            const res = await query(`INSERT INTO storage_accounts (type, name, config, is_active) 
                 VALUES ($1, $2, $3, $4) RETURNING id`, ['onedrive', name, config, false]);
    ...
    async addAliyunOSSAccount(name, region, accessKeyId, accessKeySecret, bucket) {
        ...
        const res = await query(`INSERT INTO storage_accounts (type, name, config, is_active) 
             VALUES ($1, $2, $3, $4) RETURNING id`, ['aliyun_oss', name, config, false]);
